...
def load_subdag(parent_dag_name, child_dag_name, args):
    ...
    with dag_subdag:
        return_sensor_tasks() \
        >> PythonOperator(task_id='print_result', python_callable=print_jobs_res, provide_context=True) \
        >> BashOperator(task_id='remove_run_file',
                        bash_command=f'echo Remove file {trigger_file} && rm {trigger_file}') \
        >> BashOperator(task_id='create_finished_timestamp',
                        bash_command='touch ' + trigger_dir + '/finished_{{ ts_nodash }}')   # Try format string and be amazed ! %)))
    return dag_subdag
