...
#import sys
#print('INFOOOOO:',sys.path)
from  abe_postgres import PostgreSQLCountRows
...
config = {
    'GRIDU_abe_job_dag_id_1': {'schedule_interval': None, 'start_date': airflow.utils.dates.days_ago(2),
                               'schema_name': 'public', 'table_name': 'abe_job_1'},
    'GRIDU_abe_job_dag_id_2': {'schedule_interval': None, 'start_date': airflow.utils.dates.days_ago(3),
                               'schema_name': 'public', 'table_name': 'abe_job_2'},
    'GRIDU_abe_job_dag_id_3': {'schedule_interval': None, 'start_date': airflow.utils.dates.days_ago(4),
                               'schema_name': 'public', 'table_name': 'abe_job_3'}
}
...
def push_xcom_query(**kwargs):
    ...
    sql4query = 'SELECT COUNT(*) cnt FROM {};'.format(kwargs['table_name']) # AWFUL I KNOW %)))
...
def create_dag(dag_id, dagconf, default_args):
    ...
    with  DAG(
            ...
            >> BranchPythonOperator(
                ...
                # trying op_args usage for research reason
                op_args=[postgres_def_conn_id, postgres_def_schema_name, table_name])
            ...
            >> PostgresOperator(task_id='insert_new_row', postgres_conn_id=postgres_def_conn_id,
                                trigger_rule='none_failed',
                                sql=sql4insert,
                                parameters=[uuid.uuid4().int % 123456789, datetime.now()])
            >> PythonOperator(task_id='query_new_row',
                              python_callable=push_xcom_query,
                              op_kwargs={'dag_id': dag_id, 'postgres_conn_id': postgres_def_conn_id,
                                         # trying kwargs for research reason
